...
export const buildChecksForContainer = (
  itemToCheck: Container,
  containerName: string | null,
  isQuickCheck: boolean
): Review[] => {
  const verifications: Review[] = [];
  const location =
    (containerName ? containerName + " > " : "") + itemToCheck.name;

  const isEmpty = itemToCheck.content.length === 0;
  if (!isEmpty) {
    const hasLeaf = itemToCheck.content.some(
      (child) => (child as Container).content === undefined
    );
    if (hasLeaf) {
      verifications.push({
        location,
  ...
  containerChildren.forEach((prop) => {
    const childrenChecks = buildChecksForContainer(
      prop,
      itemToCheck.name,
      isQuickCheck
    ).filter((review) =>
      isQuickCheck ? review.type !== "date" || review.mandatory : true
    );
    verifications.push(...childrenChecks);
  });

  const itemChildren = itemToCheck.content.filter((item) => !isContainer(item));
  itemChildren.forEach((prop) => {
...
export const getContentChecklist = (itemToCheck: Container) => {
  return itemToCheck.content.reduce<Array<string>>(
    (previousValue: string[], currentValue: Prop) => {
      return isContainer(currentValue)
        ? previousValue
        : [`${currentValue.number} x ${currentValue.name}`, ...previousValue];
    },
    []
  );
};
